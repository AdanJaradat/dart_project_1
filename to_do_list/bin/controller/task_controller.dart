...
//!
      ...
      DateFormat dateToFormat = DateFormat('dd/MM/yyyy HH:mm a');
      DateTime dateTimeEnd = dateToFormat.parse(end);
      //task.dateTo = dateTimeEnd;
      if (overlaps(dateTimeFrom, dateTimeEnd)) {
        task.dateFrom = dateTimeFrom;
        task.dateTo = dateTimeEnd;
        taskList.add(task);
      } else {
        color('this is conflict time.', front: Styles.BG_RED, isBold: true);
        Task.count--;
      }
  ...
  bool overlaps(DateTime start, DateTime end) {
    for (var i = 0; i < taskList.length; i++) {
      if (taskList[i].dateFrom.isBefore(end) &&
          taskList[i].dateTo.isAfter(start)) {
        return false; //dont add task
      }
    }
    //return dateTimeFrom.isBefore(end) && dateTimeEnd.isAfter(start);
    return true; //add task
  }
  ...
  void showData() {
    try {
      ...
      Date : ${_listDateTime}
      Start Time : ${taskList[i].dateFrom.hour}:${taskList[i].dateFrom.minute}
      End Time : ${taskList[i].dateTo.hour}:${taskList[i].dateTo.minute}
  ...
  void updateStartTime(int number) {
    try {
      ...
      taskList.forEach((element) {
        if (element.number == number) {
          //&& overlaps(element.dateFrom, element.dateTo)
